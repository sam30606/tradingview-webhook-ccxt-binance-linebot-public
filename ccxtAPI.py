...
def usdtBalance():
    ...
    balance=(math.floor(balance/3.02*100)/100)
...
def positions(symbol):
    position=exchange.fetchPositions(symbols=[symbol])[0] #GetPositions
    return position

def setLever(lever,symbol):
    exchange.setLeverage(lever,symbol)
